...
const {group} = useParams()
  const saveNewPackage = async(e) => {
      ...
      try {
        const response = await fetch("https://camp-coding.online/Teacher_App_2024/Mahmoud_Ammar/doctor/home/add_package.php", {
          method: "POST",
          body:  JSON.stringify({
      ...
      <form onSubmit={(e) => saveNewPackage(e)} className="animated-form">
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) => setPackageData({ ...packageData, end_date: moment(e.target.value).format() })}
            className="form-input"
          />
        </div>
